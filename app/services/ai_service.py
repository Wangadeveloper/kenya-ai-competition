...
class AIService:
    ...
    def generate_masumi_credit_report(self, farmer, officer):
        """Generates a sophisticated agricultural credit risk report for Masumi monetization sandbox."""
        # Get farmer's ledger entries and yields
        ledger_entries = farmer.ledger_entries.all() if hasattr(farmer, 'ledger_entries') else []
        yield_records = farmer.yields.all() if hasattr(farmer, 'yields') else []
        
        ledger_text = "; ".join([
            f"{e.activity_date}: {e.record_type.upper()} KES {e.amount} ({e.category} - {e.description or ''}) [Compliance: {e.compliance_status}]"
            for e in ledger_entries
        ]) or "No ledger entries recorded."
        
        yield_text = "; ".join([
            f"{y.season_name}: {y.crop_name} harvested {y.yield_kg}kg, revenue KES {y.revenue}"
            for y in yield_records
        ]) or "No yield records recorded."

        prompt = f"""
        Evaluate agricultural credit risk for farmer {farmer.full_name}:
        - Location: {farmer.county} County, Sub-county: {farmer.sub_county}
        - Farm Size: {farmer.farm_size} Acres
        - Primary Crop: {farmer.primary_crop or 'Maize'}
        - Water Source: {farmer.water_source}
        - Historical Farm Ledger: {ledger_text}
        - Yield Records: {yield_text}
        ...
        try:
            ...
            print("Error in Gemini Masumi Report generation:", e)
            # Simulated rules-engine fallback
            score = 50
            ...
            flagged_count = sum(1 for e in ledger_entries if e.compliance_status == 'Flagged')
            ...
            if getattr(farmer, 'years_farming', 0) > 5:
                score += 10
            ...
            crop = farmer.primary_crop or 'Maize'
            strengths = ["Established farming footprint", f"Experienced in {crop} cultivation"]
            if total_income > total_expense:
                strengths.append("Positive farm ledger net cash flow")
                
            vulnerabilities = []
            if farmer.water_source == 'Rain-fed':
            ...
            if not vulnerabilities:
                vulnerabilities.append("Market price volatility exposures")
                
            return {
                "credit_score": score,
                "risk_level": risk_level,
                "recommended_loan_cap": recommended_loan,
                "narrative_summary": f"Mkulima {farmer.full_name} is operating a {farmer.farm_size}-acre {crop} farm in {farmer.county}. Financial cash flows are moderate, with compliance checks indicating key risk metrics are stabilized. Operational credit tier remains {risk_level} risk.",
                "strengths": strengths,
