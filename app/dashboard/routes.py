...
from app.models.sql_models import User, FarmPlan, LoanApplication, Post, FieldVisit, Sacco, FarmLedger, CropYield, MasumiTransaction
...
def plan_season():
    if request.method == 'POST':
        ...
        ai = AIService(user_context=current_user)
        ...
        new_plan = FarmPlan(
            ...
            season_name=f"Long Rains Season {datetime.now(timezone.utc).year}",
...
def log_visit():
    ...
    condition = request.form.get('crop_health_status')
...
# =========================================================================
# 🤖 UPGRADED MULTIMODAL AGENTIC DOCUMENT ROUTE
# =========================================================================
@dashboard_bp.route('/scan-input', methods=['POST'])
@login_required
def scan_input_compliance():
    """
    Unified agent endpoint. Accepts both high-volume invoice receipts or single product labels.
    The AI system reads the document type and handles item logs, safety overrides, and messaging loops.
    """
    if current_user.role != 'farmer':
        flash('Access denied. Only registered farmers can process input sheets.', 'danger')
        return redirect(url_for('dashboard.index'))

    file = request.files.get('input_label_image')
    if not file or file.filename == '':
        flash('Please upload a legible asset image file or purchase receipt document.', 'danger')
        return redirect(url_for('dashboard.index'))
    ...
    try:
        ...
        # Instantiate AI service passing the request context user entity
        ai_agent = AIService(user_context=current_user)
        
        # Execute the main semantic tool orchestration loop
        agent_narrative_feedback = ai_agent.execute_multimodal_document_agent(
            image_bytes=image_bytes,
            mime_type=mime_type,
            target_crop=current_user.primary_crop or 'Maize'
        )
        
        flash(f"Uchambuzi wa Kidijiti: {agent_narrative_feedback}", 'success')
        
    except Exception as e:
        print(f"Document Agent Dispatch System Error: {e}")
        flash(f'Fatal scanning engine error encountered: {str(e)}', 'danger')
...
def officer_copilot(farmer_id):
    ...
    3. The recommended focus area for the upcoming visit.
...
@dashboard_bp.route('/officer/masumi')
@login_required
def officer_masumi_hub():
    if current_user.role != 'officer':
        flash('Access restricted to official field staff identifiers.', 'danger')
        return redirect(url_for('dashboard.index'))
    
    # Fetch all farmers in the officer's region
    farmers = User.query.filter_by(role='farmer', county=current_user.county).all()
    # Fetch all Masumi transactions made by this officer
    transactions = MasumiTransaction.query.filter_by(officer_id=current_user.id).order_by(MasumiTransaction.created_at.desc()).all()
    
    return render_template(
        'dashboard/officer_masumi.html',
...
@dashboard_bp.route('/analysis/request', methods=['POST'])
@login_required
def analysis_request():
    ...
    if not farmer_id:
        return jsonify({"error": "Missing farmer_id parameter"}), 400
    ...
    # Check if officer belongs to platform owner (Mercy Corps)
    is_owner = current_user.organization and current_user.organization.strip().lower() == 'mercy corps'
    
    if is_owner:
        # Free path: immediately run credit report
        try:
            ai = AIService(user_context=current_user)
            report = ai.generate_masumi_credit_report(farmer, current_user)
            
            tx = MasumiTransaction(
            ...
            return jsonify({
                ...
                "message": "Free access granted to platform owner (Mercy Corps). Report generated.",
                "report": report,
        ...
        # Paid path: requires Masumi payment request
        tx = MasumiTransaction(
            payment_id=payment_id,
            officer_id=current_user.id,
            farmer_id=farmer.id,
        ...
        return jsonify({
            ...
            "amount": 2.50,
            "organization": current_user.organization,
            "message": "Masumi payment request created. Settlement token required.",
            "transaction_id": tx.id
        })


@dashboard_bp.route('/payment/complete', methods=['POST'])
@login_required
def payment_complete():
    ...
    tx = MasumiTransaction.query.filter_by(payment_id=payment_id, officer_id=current_user.id).first()
    if not tx:
    ...
    if tx.status == 'completed':
        return jsonify({
            ...
            "report": tx.report_payload,
            "transaction_id": tx.id
        })
        
    # Transition to paid, then complete the report
    tx.status = 'paid'
    db.session.commit()
    
    try:
        # Trigger Gemini credit risk report evaluation on raw ledger data
        ai = AIService(user_context=current_user)
        report = ai.generate_masumi_credit_report(tx.farmer, current_user)
        
        tx.status = 'completed'
        tx.report_payload = report
        db.session.commit()
        ...
        return jsonify({
            ...
            "payment_id": tx.payment_id,
            "amount": tx.amount,
            "message": "Payment cleared via Masumi. Credit risk analysis completed.",
            "report": report,
            "transaction_id": tx.id
        })
    except Exception as e:
        tx.status = 'failed'
        db.session.commit()
        return jsonify({"error": f"Failed during AI evaluation: {str(e)}"}), 500


@dashboard_bp.route('/payment/<string:payment_id>')
@login_required
def get_payment_status(payment_id):
    if current_user.role != 'officer':
        return jsonify({"error": "Unauthorized"}), 403
        
    tx = MasumiTransaction.query.filter_by(payment_id=payment_id, officer_id=current_user.id).first_or_404()
    return jsonify({
        "payment_id": tx.payment_id,
        "farmer_name": tx.farmer.full_name,
        "amount": tx.amount,
        "status": tx.status,
        "created_at": tx.created_at.strftime('%Y-%m-%d %H:%M:%S'),
        "has_report": tx.report_payload is not None
    })


@dashboard_bp.route('/analysis/report/<int:transaction_id>')
@login_required
def view_masumi_report(transaction_id):
    if current_user.role != 'officer':
        flash('Unauthorized access.', 'danger')
        return redirect(url_for('dashboard.index'))
        
    tx = MasumiTransaction.query.filter_by(id=transaction_id, officer_id=current_user.id).first_or_404()
    if not tx.report_payload:
        flash('Report payload is not yet unlocked or generated.', 'warning')
        return redirect(url_for('dashboard.officer_masumi_hub'))
        
    return render_template('dashboard/masumi_report_view.html', tx=tx, report=tx.report_payload)